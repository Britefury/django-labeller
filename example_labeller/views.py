...
def home(request):
    ...
    return render(request, 'example_labeller/index.html', context)
                                ...
                                if entry['labels'] is not None:
                                    ...
                                    with z.open(entry['labels'], mode='r') as f_labels:
                                        try:
                                            ...
                                            # Unwrap the labels
                                            # _unwrap_labels() returns ['finished'] if json "complete": True, else: []
...
class LabellingToolAPI (labelling_tool_views.LabellingToolViewWithLocking):
    def get_labels(self, request, image_id_str, *args, **kwargs):
        ...
        print(f' GET request->LabellingToolAPI image_id={image_id_str}')
        return image.labels
...
def delete_image(request):
    ...
    if request.method == 'POST':
        ...
        # TODO if no more images left, redirect to home
        #if (int(image_id_str) - 1) == 0:
        #    return redirect('example_labeller:go_home')
